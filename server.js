...
server.listen(port, () => {
  console.log(`✅ Servidor rodando em http://localhost:${port}`);
      ...
      const newUser = { id: Date.now(), nome, email, senha, token: createToken() };
      users.push(newUser);
      writeUsers(users);
      sendJson(res, 201, { ok: true, user: { id: newUser.id, nome, email }, token: newUser.token });
    } catch (error) {
  ...
  if (req.method === 'POST' && requestUrl.pathname === '/api/checkout') {
    try {
      const { token, items } = await parseBody(req);
      if (!token) {
        sendJson(res, 401, { ok: false, message: 'Token ausente.' });
        return;
      }
      ...
      sendJson(res, 200, { ok: true, message: 'Compra finalizada com sucesso.', items });
    } catch (error) {
      sendJson(res, 400, { ok: false, message: 'JSON inválido.' });
    }
    return;
